...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/Users/srichandrasamanapalli/Library/Application Support/tldraw/server.json`.
...
```bash
PORT=$(jq -r .port '/Users/srichandrasamanapalli/Library/Application Support/tldraw/server.json'); TOKEN=$(jq -r .token '/Users/srichandrasamanapalli/Library/Application Support/tldraw/server.json')
